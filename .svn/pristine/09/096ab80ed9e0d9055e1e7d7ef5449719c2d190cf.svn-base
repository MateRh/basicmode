...
_initialLoading[ "shaders/car_reflect/c_car_reflect.lua" ] = function ( )
...
local cprEffectEnabled, grunShader, geneShader, shatShader, myScreenSource, textureVol, vehTimer
...
local function setEffectV()
	...
	v.renderDistance = 5000 -- shader will be applied to textures nearer than this

	v.sparkleSize = 0.5	-- scale the carpaint sparkles
	v.brightnessFactorPaint = 0.008
	v.brightnessFactorWShield = 0.099
	
	v.bumpSize = 0.5 -- for car paint
	v.bumpSizeWnd = 0 -- for windshields
	v.normal = 0.95 -- deformation strength (0-2.0) 2.0 = the highest
...
local function startCarPaintReflect()
		if cprEffectEnabled then return end

			if not fileExists( "shaders/car_reflect/images/smallnoise3d.dds" ) then
				if not downloading then
				--	initiateFileTransfer( "shaders/car_reflect/images/smallnoise3d.dds", 250000 )
				addEventHandler ( "onClientFileDownloadComplete", getRootElement(), onFileDownFinished )
				downloadFile( "shaders/car_reflect/images/smallnoise3d.dds" )

				downloading = true
				--	dTimer = setTimer( startCarPaintReflect, 5000, 1 )
				end
				return 0;
			end	
			downloading = false
		...
		-- Create stuff
		local _xxxx 
		grunShader, _xxxx = dxCreateShader ( "shaders/car_reflect/fx/car_refgrun.fx",1,v.renderDistance,false)

		geneShader, _xxxx = dxCreateShader ( "shaders/car_reflect/fx/car_refgene.fx",1,v.renderDistance,true)

		shatShader, _xxxx = dxCreateShader ( "shaders/car_reflect/fx/car_refgene.fx",1,v.renderDistance,true)
		...
		--	outputConsole( "Started: Shader Car paint reflect.")
			...
			-- Set texture
			textureVol = dxCreateTexture ( "shaders/car_reflect/images/smallnoise3d.dds" )
...
local function stopCarPaintReflect()
...
local function onFileDownFinished_( file, success )
	...
	if file == "shaders/car_reflect/images/smallnoise3d.dds" then
		if success then
			removeEventHandler ( "onClientFileDownloadComplete", getRootElement(), onFileDownFinished )
			setTimer( startCarPaintReflect, 500, 1 )
...
function switchCarPaintReflect( cprOn )
	...
	_initialLoaded( "shaders/car_reflect/c_car_reflect.lua" )	
