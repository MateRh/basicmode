...
PSInput VertexShaderFunction(VSInput VS)
{
    ...
    PS.NormalCl = float3(0,VS.Normal.yz);
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    float2 Nn = PS.NormalCl.xyz/(length(float3(PS.NormalCl.xy* sNorFac, PS.NormalCl.z)));
    float2 Vn = float2(PS.TexCoord.x,PS.TexCoord.y);
...
// Techniques
//------------------------------------------------------------------------------------------
technique car_reflect_generic_SM3
{
    pass P0
    {
        DepthBias = -0.0003;
...
technique car_reflect_generic_SN2
{
    pass P0
    {
        DepthBias = -0.0003;
